import { last, remove } from 'es-toolkit/compat'
import {
  computed,
  onUnmounted,
  shallowReactive,
...
export const useZIndex = (
...
): [index: ComputedRef<number>, isLast: ComputedRef<boolean>, stopUse: () => void] => {
  ...
  return [
    computed(() => (allLayers.indexOf(th) + 1) * 10),
    computed(() => last(allLayers) === th),
    stop
  ]
}
